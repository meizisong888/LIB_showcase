...
export function FindSkillPage() {
  return (
    <>
      ...
      <section className="evaluation-steps section"><div className="container"><div className="section-heading split-heading"><div><p className="eyebrow">Eight-step review</p><h2>From task definition to reversible test</h2></div><p>Do these in order. A later safety check cannot undo data already exposed during installation or authorization.</p></div><ol>{steps.map((step, index) => { const Icon = step.icon; return <li key={step.title}><span className="step-number">{String(index + 1).padStart(2, '0')}</span><span className="step-icon"><Icon /></span><div><h3>{step.title}</h3><p>{step.text}</p></div></li> })}</ol></div></section>
